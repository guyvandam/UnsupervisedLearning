...
class LoadDataSet2(LoadData):
    ...
    def __init__(self, nrows=None):
        super().__init__(set2Path, ",",datasetIndex=2, nrows=nrows)
    ...
    def prepareDataset(self):
        ...
        allNumberColumns = ['admission_type_id','discharge_disposition_id','admission_source_id','time_in_hospital','num_lab_procedures','num_procedures','num_medications','number_outpatient','number_emergency','number_inpatient','number_diagnoses']
        ...
        for columnIndex in self.dataFrame.columns:
            if not columnIndex in allNumberColumns:
                ...
                if len(unique) == 1 or columnIndex in columnsToDelete: # only one value in a column. Doesn't tell us anything about that data.
                    del self.dataFrame[columnIndex]
                    continue

                self.dataFrame[columnIndex] = self.dataFrame[columnIndex].map(
                    dict(zip(unique, range(len(unique)))))
